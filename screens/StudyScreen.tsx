...
const StudyScreen: React.FC = () => {
  ...
  const [videoEnded, setVideoEnded] = useState(false); // 👈 NOVO estado
  ...
  useEffect(() => {
    setStudyStep('video');
    setVideoEnded(false); // 👈 reseta toda vez que muda de etapa
  }, [currentStageId]);
  ...
  const completedStagesCount = useMemo(
    () => Object.values(stageProgress).filter(p => (p as StageProgress).completed).length,
    [stageProgress]
  );

  const progressPercentage = stagesData.length > 0 
        ...
        return (
          <div className={`w-full max-w-4xl p-6 bg-gray-800 bg-opacity-70 backdrop-blur-sm rounded-2xl shadow-2xl border ${theme.cardBorder} text-white text-center animate-fade-in`}>
            <div className={`aspect-w-16 aspect-h-9 w-full rounded-lg overflow-hidden shadow-lg mb-6 border-2 ${theme.cardBorder}`}>
              {
                /\.mp4($|\?)/i.test(currentStageData.videoUrl) ? (
                  <video
                    ...
                    playsInline
                    preload="metadata"
                    onEnded={() => setVideoEnded(true)} // 👈 quando termina o vídeo
                  />
                ) : (
                  <iframe
            ...
            {/* 👇 Só mostra o botão se o vídeo terminou */}
            {videoEnded ? (
              ...
              <div key={stage.id} className="flex-1 text-center font-bold">
                <span
                  className={`px-2 py-1 rounded-full transition-colors ${
                    currentStageId === stage.id ? `text-white ${theme.accentBg}` : ''
